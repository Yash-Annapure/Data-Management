...
def process_employee_data(df):
    # 1. Computing average salary of all employees (before filtering)
    avg_salary = df['Salary'].mean()
    
    # 2. Finding oldest employee (before filtering)
    # idxmax() finds index of the row with the highest Age
    oldest_idx = df['Age'].idxmax()
    oldest_name = df.loc[oldest_idx, 'Employee_Name']
    oldest_dept = df.loc[oldest_idx, 'Department']
    
    # 3. Filtering employees from IT department
    df_filtered = df[df['Department'] == 'IT'].copy()
    
    # 4. Returning results as new columns
